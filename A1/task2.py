...
class AlgorithmBatched:
    def __init__(self, num_arms, horizon, batch_size):
        ...
        #I am implementing thompson model
    ...
    def give_pull(self):
        # START EDITING HERE
        x = np.argmax(np.random.beta(self.success+1,self.failures+1,size=(self.batch_size,self.num_arms)),1)
        indeces,values = np.unique(x,return_counts=True)
    ...
    def get_reward(self, arm_rewards):
        ...
        for arm in arm_rewards:
            unique,counts = np.unique(arm_rewards[arm],return_counts=True)
            x = dict(zip(unique, counts))
            if 1 in x.keys():
                self.success[arm] += x[1]
            if 0 in x.keys():
                self.failures[arm] += x[0]
        # END EDITING HERE