...
class UCB(Algorithm):
    ...
    def give_pull(self):
        ...
        if self.time< self.num_arms:
            ...
            ucb = self.values + np.sqrt((2*np.log(self.time))*np.reciprocal(self.counts))
    ...
    def give_pull(self):
        ...
        if self.time < self.num_arms:
            ...
            for arm in range(self.num_arms):
                pa = self.values[arm]
                final_term = math.log(self.time)+3*math.log(math.log(self.time))
                start = pa
                end = 1
                left_term=0
                right_term=0
                ...
                while end-start>0.001:
                    ...
                    if pa==0:
                        ...
                        left_term = pa*math.log(pa/q)
                    right_term = (1-pa)*math.log((1-pa)/(1-q))
                    if self.counts[arm]*(left_term+right_term) > final_term:
                        end = q
                    else :
                        start = q
                kl_ucb[arm] = end
